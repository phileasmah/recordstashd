...
export function useSpotify() {
  ...
  const searchArtistAndAlbums = useCallback(async (query: string, limit: number = 5) => {
    return fetchWithToken(`/search?q=${encodeURIComponent(query)}&type=artist,album&limit=${limit}`);
  }, [fetchWithToken]);
